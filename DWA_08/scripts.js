...
const displayBookWithElements = (book) => {
  ...
  bookContainer.addEventListener("click", () => {
    BookPreviewModule.createBookPreview(book);
    BookPreviewModule.openBookDetails();
  });
...
/**
 * This is a module for managing the book preview functionality.
 */

const BookPreviewModule = {

 /**
   * Populate the book details in the preview modal overlay.
   * This function sets the book details based on the provided book object.
   *
  ...
  createBookPreview: (book) => {
    ...
    htmlElements.preview.subtitle.textContent = `${authorName} (${year})`;
    htmlElements.preview.description.textContent = description;
  },
   /**
     * This function opens the book details modal overlay and focuses on it.
    */
  openBookDetails: () => {
  ...
  /**
     * This function closes the book details overlay.
    */
  closeBookDetails: () => {
    htmlElements.preview.overlay.close();
  },
};
// Event listener to create book preview and open details when a book is clicked
htmlElements.main.items.addEventListener('click', () => {
  BookPreviewModule.createBookPreview();
  BookPreviewModule.openBookDetails();
});
// Event listener to close the book details modal overlay
htmlElements.preview.close.addEventListener('click', () => {
  BookPreviewModule.closeBookDetails();
});
